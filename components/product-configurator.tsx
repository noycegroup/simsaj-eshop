...
export function ProductConfigurator({ product }: Props) {
  ...
  return (
    <div className="product-configurator">
      ...
      {product.widths.length ? <fieldset><legend>Vyberte šírku</legend><div className="width-grid">{product.widths.map((value) => <button className={width === value.code ? "selected" : ""} type="button" onClick={() => setWidth(value.code)} key={value.code}>{value.label}</button>)}</div></fieldset> : null}
