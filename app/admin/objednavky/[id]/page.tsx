...
type EmailPreview = { id: string; audience: "customer" | "admin"; recipient_email: string | null; subject: string; body_text: string; status: string; attempt_count: number; last_error: string | null; sent_at: string | null; created_at: string };
...
export default async function AdminOrderDetailPage({ params }: PageProps) {
  ...
  const user = await requireChatGPTUser(`/admin/objednavky/${id}`);
  ...
  const emails = ((order as Order & { order_email_notifications?: EmailPreview[] }).order_email_notifications ?? []);
  const shipping = address(order.shipping_address);
  const actions = order.is_test ? nextStatuses[order.status] ?? [] : [];

  return <><SiteHeader suggestions={[]} /><main className="admin-page"><AdminToolbar label="Detail objednávky" userName={user.displayName} signOutHref={chatGPTSignOutPath("/")} backHref="/admin/objednavky" backLabel="Všetky objednávky" /><section className="admin-shell order-detail-shell">
    ...
    <section className="admin-panel order-email-previews"><div className="admin-panel-heading"><div><h2>Náhľady e-mailov</h2><p>Skúšobný režim – tieto správy sa neodosielajú.</p></div><span className="admin-badge test">Koncepty</span></div><div className="email-preview-grid">{emails.map((email) => <details key={email.id}><summary><span><strong>{email.audience === "customer" ? "Potvrdenie zákazníkovi" : "Notifikácia administrátorovi"}</strong><small>{email.recipient_email ?? "Interný príjemca bude nastavený neskôr"}</small></span><em>{email.status === "draft" ? "Neodoslané" : email.status}</em></summary><div><h3>{email.subject}</h3><pre>{email.body_text}</pre></div></details>)}</div></section>
  </section></main></>;
}
